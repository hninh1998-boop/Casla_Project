sap.ui.define([
    ...
    /**
     * Base cho controller extension "View PDF" trên ListReport (Fiori Elements V4).
     * Copy nguyên file này sang app mới, rồi tạo 1 file PDF.controller.js nhỏ gọi
     * createPdfViewerController(sControllerName, oConfig) với oConfig riêng của app đó.
     *
     * oConfig:
     *   actionName      {string}   Tên đầy đủ của bound action, vd:
     *                               "com.sap.gateway.srvd.<service>.v0001.btnPrintPDF"
     *   getDedupeKey    {function} (oContextObject) => string. Key để lọc trùng file
     *                               khi nhiều dòng được chọn cùng thuộc 1 chứng từ/PDF.
     *   fileNamePrefix  {string}   Tiền tố tên file khi backend không trả fileName.
     *   collectParams   {function=} () => Promise<object>. Optional - nếu action cần
     *                               mở dialog thu thập tham số trước khi gọi (giống PI
     *                               Quality/Packing/Payment ở ord_cfm). Bỏ qua nếu action
     *                               không cần tham số nào (case phổ biến nhất).
     */
    return function createPdfViewerController(sControllerName, oConfig) {

        return ControllerExtension.extend(sControllerName, {
            ...
            btnPrintPDF: function () {
                ...
                if (oConfig.collectParams) {
                    oConfig.collectParams.call(this).then(function (oParams) {
                        that._callPrintPdfAction(aContexts, oParams);
                    }, function () {
                        // User huỷ dialog - không làm gì
                    });
                } else {
                    this._callPrintPdfAction(aContexts, null);
                }
            },
            ...
            _callPrintPdfAction: function (aContexts, oParams) {
                ...
                aContexts.forEach(function (oContext) {
                    var oOperation = oModel.bindContext(oConfig.actionName + "(...)", oContext, {
                        $$groupId: sGroupId
                    });
                    ...
                    var pExecute = oOperation.execute().then(function () {
                        ...
                        return {
                            dedupeKey: oConfig.getDedupeKey(oContext.getObject()),
                            result: oResult
                        };
                    });
                ...
                Promise.all(aPromises).then(function (aResults) {
                    ...
                    // 5. Duyệt kết quả trả về để LỌC TRÙNG THEO DEDUPE KEY
                    aResults.forEach(function (oItem) {
                        var sKey = oItem.dedupeKey;
                        var oResult = oItem.result;

                        if (oResult && oResult.fileContent && !oSeenKeys[sKey]) {
                            ...
                            aPdfFiles.push({
                                name: oResult.fileName || (oConfig.fileNamePrefix + sKey),
