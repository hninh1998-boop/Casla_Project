sap.ui.define([
    ...
    Button, Label, TextArea, Text, Item, HTML
) {
    'use strict';

    return ControllerExtension.extend('ordcfm.ext.controller.PDF', {
        ...
        /* ================================================================
         *  DIALOG NHẬP THAM SỐ - giữ nguyên 100%, chỉ dùng để thu thập Quality/Packing/Payment
         * ================================================================ */
        _openParamDialog: function () {
            return new Promise(function (resolve, reject) {
                var oQuality = new TextArea({ width: "100%", rows: 2 });
                var oPacking = new TextArea({ width: "100%", rows: 2 });
                var oPayment = new TextArea({ width: "100%", rows: 2 });

                var oDialog = new Dialog({
                    title: "Thông tin in PI (Proforma Invoice)",
                    contentWidth: "500px",
                    content: [
                        new VBox({
                            items: [
                                new Label({ text: "Quality", labelFor: oQuality }),
                                oQuality,
                                new Label({ text: "Packing", labelFor: oPacking }).addStyleClass("sapUiTinyMarginTop"),
                                oPacking,
                                new Label({ text: "Payment", labelFor: oPayment }).addStyleClass("sapUiTinyMarginTop"),
                                oPayment
                            ]
                    ...
                    beginButton: new Button({
                        ...
                        press: function () {
                            var oResult = {
                                Quality: oQuality.getValue(),
                                Packing: oPacking.getValue(),
                                Payment: oPayment.getValue()
                            };
                            oDialog.close();
                            resolve(oResult);
                        }
        ...
        /* ================================================================
         *  MAIN ACTION - btnPrintPDF
         * ================================================================ */
        btnPrintPDF: function () {
            var that = this;

            // 1. Lấy tất cả các dòng đã chọn (Giữ nguyên full data không lọc trước)
            var aContexts = this._getSelectedContexts();
            if (!aContexts.length) {
                sap.m.MessageToast.show("Vui lòng chọn ít nhất 1 dòng.");
                return;
            }

            // 2. Mở dialog nhập tham số và truyền FULL mảng các dòng xuống backend
            this._openParamDialog().then(function (oParams) {
                that._callPrintPdfAction(aContexts, oParams);
            }, function () {
         ...
         * ================================================================ */
        _callPrintPdfAction: function (aContexts, oParams) {
            var that = this;
            var oModel = this.base.getExtensionAPI().getModel();
            
            var oBusy = new sap.m.BusyDialog({ text: "Đang xử lý " + aContexts.length + " dòng..." });
            oBusy.open();
            ...
            aContexts.forEach(function (oContext) {
                // Cấu hình tham số $$groupId để UI5 biết các action này đi chung một chuyến xe
                var oOperation = oModel.bindContext("com.sap.gateway.srvd.zsd_ord_conf_item.v0001.btnPrintPDF(...)", oContext, {
                    $$groupId: sGroupId
                });
                
                oOperation.setParameter("Quality", oParams.Quality);
                oOperation.setParameter("Packing", oParams.Packing);
                oOperation.setParameter("Payment", oParams.Payment);

                // Tạo Promise xử lý kết quả cho từng dòng sau khi Batch chạy xong
                var pExecute = oOperation.execute().then(function () {
                    var oResult = oOperation.getBoundContext().getObject();
                    return {
                        salesOrder: oContext.getObject().SalesOrder,
                        result: oResult
                    };
                });
            ...
            // Lúc này, internal table `keys` bên ABAP của RAP sẽ nhận ĐẦY ĐỦ số dòng cậu đã chọn
            ...
            Promise.all(aPromises).then(function (aResults) {
                ...
                var aPdfFiles = [];
                var oSeenSalesOrders = {};

                // 5. Duyệt kết quả trả về để LỌC TRÙNG THEO SALES ORDER
                aResults.forEach(function (oItem) {
                    var sSalesOrder = oItem.salesOrder;
                    var oResult = oItem.result;

                    // Nếu dòng này có file và Sales Order này CHƯA được đưa vào danh sách hiển thị
                    if (oResult && oResult.fileContent && !oSeenSalesOrders[sSalesOrder]) {
                        
                        oSeenSalesOrders[sSalesOrder] = true; // Đánh dấu SO này đã lấy file thành công

                        var conv = that._b64ToSources(oResult.fileContent, oResult.mimeType || "application/pdf");
                        
                        aPdfFiles.push({
                            name: oResult.fileName || ("PI_" + sSalesOrder),
                ...
                // 6. Mở Dialog hiển thị danh sách PDF (Dropdown Select giờ chỉ hiện đúng 2 file gọn gàng)
                if (aPdfFiles.length > 0) {
                    sap.m.MessageToast.show("Đã tạo thành công " + aPdfFiles.length + " file PDF.");
                    that._openPdfPreview(aPdfFiles);
                } else {
                    sap.m.MessageBox.warning("Không có nội dung file nào được trả về.");
                }

            }).catch(function (oError) {
                oBusy.close();
                console.error("[btnPrintPDF Batch Multi]", oError);
                sap.m.MessageBox.error(oError && oError.message ? oError.message : "Đã xảy ra lỗi khi tạo PDF.");
            });
        },
