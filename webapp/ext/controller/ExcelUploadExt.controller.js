sap.ui.define([
	...
	return ControllerExtension.extend("zuploadzbomrp.ext.controller.ExcelUploadExt", {
		downloadTemplate: async function () {
			const [{ default: Spreadsheet }] = await Promise.all([
				new Promise(res => sap.ui.require(["sap/ui/export/Spreadsheet"], S => res({ default: S })))
			]);

			const oCols = [
				{ label: "Sales Order", property: "SalesOrder", type: "string" },
				{ label: "Sales Order Item", property: "SalesOrderItem", type: "string" },
				{ label: "Material Number*", property: "Material", type: "string" },
				{ label: "Plant*", property: "Plant", type: "string" },
				{ label: "Alternative BOM", property: "BillOfMaterialVariant", type: "string" },
				{ label: "BOM Usage*", property: "BillOfMaterialVariantUsage", type: "string" },
				{ label: "BOM Item Number*", property: "BillOfMaterialItemNumber", type: "string" },
				{ label: "Item Category*", property: "BillOfMaterialItemCategory", type: "string" },
				{ label: "BOM Component", property: "Component", type: "string" },
				{ label: "Component Quantity*", property: "ComponentQuantity", type: "string" },
				{ label: "Component Unit of Measure", property: "ComponentUnit", type: "string" },
				{ label: "Indicator: Net Scrap", property: "IsNetScrap", type: "string" },
				{ label: "Component Scrap in Percent", property: "ComponentScrapInPercent", type: "string" },
				{ label: "Indicator: Relevancy to Costing", property: "BomItemIsCostingRelevant", type: "string" },
				{ label: "Special procurement", property: "SpecialProcurementType", type: "string" },
				{ label: "Issue Location for Production Order", property: "ProdOrderIssueLocation", type: "string" }
			];

			const oSettings = {
				workbook: { columns: oCols },
				dataSource: [{}],
				fileName: "Template_AddBomItem.xlsx",
				showProgress: false
			};

			const oSheet = new Spreadsheet(oSettings);
			oSheet.build()
				.then(() => MessageToast.show("Template downloaded successfully."))
				.catch(e => MessageToast.show("Download failed: " + e))
		...
		_NS: "com.sap.gateway.srvd.zui_upload_zbom_rp.v0001.",
		_ENTITY_SET: "ManageFile",
		_DIALOG_ID: "idFileUploadDialog",
		_FRAGMENT: "zuploadzbomrp.ext.fragment.filedialog",
		...
		override: {
			...
			editFlow: {
				onAfterActionExecution: function (oEvent) {
					// oEvent là string dạng: "com.sap.gateway.srvd.zui_upload_zbom_rp.v0001.downloadTemplate(...)"
